...
- Selection tools for nodes and wires: click, shift-click, lasso, and
  select-all.
...
- Load Transformation button in the library to import transformation JSON
  files.
...
2. **Wire nodes**: Drag from an output port (blue) to an input port
   (green).
...
5. **Select wires**: Click on a wire to select it, Shift+click to
   multi-select wires, or use lasso selection. Selected wires appear blue
   and thicker.
6. **Delete nodes/wires**: Select elements and press Delete/Backspace, or
   right-click and choose Delete from the context menu.
7. **Undo**: Press Ctrl/Cmd+Z to undo the last diagram change (up to 20
   actions).
...
10. **Palette management**: Create custom node types, then save/load
    palette JSON from the palette panel.
...
12. **Apply transformation**: Right-click a node selection and choose Apply
    Transformation from the context menu.
